 ...
 * @since 1.0.0
 */
if (!class_exists('portfoliox_About')) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class portfoliox_About
	{
		/**
		 ...
		 *
		 * @var portfoliox_About $instance The  TI_About_Page instance.
		 */
		...
		{
			if (!isset(self::$instance) && !(self::$instance instanceof portfoliox_About)) {
				self::$instance = new portfoliox_About;
		...
		public function free_pro()
		{
			...
			if (!empty($free_pro)) {
				if (!empty($free_pro['free_theme_name']) && !empty($free_pro['pro_theme_name']) && !empty($free_pro['features']) && is_array($free_pro['features'])) {
					...
					echo esc_html('PortfolioX Pro Live Preview', 'portfoliox');
					echo '</a></td></tr>';
			...
			if ('appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix) {
				...
				wp_localize_script(
					...
					array(
						...
						'activating_string'        => __('Activating', 'portfoliox')
					)
				);
			}
		}
